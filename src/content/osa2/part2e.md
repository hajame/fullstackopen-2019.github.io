...
There are many ways of matching elements using [different types of CSS selectors](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Selectors).
...
In React we have to use the [className](https://reactjs.org/docs/dom-elements.html#classname) attribute instead. With this in mind, let's make the following changes to our <i>Note</i> component:
...
If the value of the <em>message</em> prop is <em>null</em>, then nothing is rendered to the screen. In other cases the message gets rendered inside of a div element.
...
Let's add a style rule that suits an error message:
...
When an error occurs we add a descriptive error message to the <em>errorMessage</em> state. At the same time we start a timer, that will set the <em>errorMessage</em> state to <em>null</em> after five seconds.
...
<!-- CSS-säännöt määritellään Javascriptin avulla hieman eri tavalla kuin normaaleissa CSS-tiedostoissa. Jos haluamme esimerkiksi asettaa jollekin elementille vihreän, kursivoidun ja 16 pikselin korkuisen fontin, eli CSS-syntaksilla ilmaistuna -->CSS rules are defined slightly differently in JavaScript than in normal CSS files. Let's say that we'd want to make some text element green, and make the font italic size of 16 pixels. In CSS it would look like this:
...
Inline styles and some of the other ways of adding styles to React components go completely against the grain of old conventions. Traditionally it was considered the best practice to separate CSS entirely from content (HTML) and functionality (JavaScript). According to the old school of thought, the goal was to write CSS, HTML, and JavaScript into their own separate files.
...
The unit of a functional entity is the React component, which defines the structure expressed through HTML, the functionality defined as JavaScript functions, and the style all in one place, so that the individual components become as independent and reusable as possible.
